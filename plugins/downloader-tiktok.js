...
const handler = async (m, {conn, text, args, usedPrefix, command}) => {
  ...
  const texto = `*Hola 👋🏼 @${m.sender.split`@`[0]} evite jugar con el bot , mientras que envio su video soy algo lento...🐢*`;
  // let buttons = [{ buttonText: { displayText: '♫ 𝙰𝚄𝙳𝙸𝙾 ♫' }, buttonId: `${usedPrefix}tomp3` }]
  try {
    const aa = {quoted: m, userJid: conn.user.jid};
    const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: texto, contextInfo: {externalAdReply: {title: 'ᴛʜᴇ ᴍʏsᴛɪᴄ - ʙᴏᴛ', body: null, thumbnail: imagen1, sourceUrl: 'https://instagram.com/crlx.emxn'}, mentionedJid: [m.sender]}}}, aa);
    await conn.relayMessage(m.chat, prep.message, {messageId: prep.key.id, mentions: [m.sender]});
    const dataF = await tiktok.v1(args[0]);
    // let desc1 =  `*𝙽𝙸𝙲𝙺𝙽𝙰𝙼𝙴:* ${dataF.nickname || 'Indefinido'}`
    const desc1 = `*Hola👋🏼*\n*Aquí tienes tu video de TikTok , Gracias por tener paciencia*\n\n\n*Bot creado por Ezequiel y naye <3*`;
